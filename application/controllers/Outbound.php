...
class Outbound extends CI_Controller {
   ...
   public function buyer_details() {
    ...
    public function add_ticket_note() {
      ...
      if($insert_id > 0) {
       ...
       if($photo) {
            ...
            $description = $note . "<br> <a href='".$photo_url."'><b>Attachment</b></a>";
        } else {
            $description = $note;
        }
        ...
        redirect('outbound/ticket_details/'.$ticket_id, 'refresh');
      }
    ...
    public function call_tagging(){
      ...
      $insert_id = $this->Admin_model->add_call_logs($data);

      // add to logs
      $description = $this->input->post('result') . " call attempt";
      $act_data = array(
        'ticket_id' => $this->input->post('ticket_id'),
        'description' => $description,
        'created_by' => user('id'),
      ...
      if($insert_id > 0) {
        echo "<script type='text/javascript'>alert('Call Log has been saved.');</script>";
        redirect('outbound/ticket_details/'.$this->input->post('ticket_id'), 'refresh');
      }
    }
    ...
    public function add_schedule() {
       ...
       if($insert_id > 0) {
        ...
        // add to logs
        $description = "Unit Owner confirmed the turnover schedule.";
      ...
      $update_id = $this->Admin_model->add_activity_log($act_data);
            ...
            // create ticket - random assigning to HAND associate
          $o_associates = array();
