...
<div class="tab-content">
  	...
  	<div class="tab-pane container active" id="customer_details" style="padding-top: 30px;">
		...
		<div class="tab-pane container" id="tickets_list" style="padding-top: 30px;">
			<?php $tickets = $this->Admin_model->get_all_tickets_only_by_customer_number($this->uri->segment(3)); ?>
			<?php if($tickets) :?>
				<div class="row">
				<?php foreach($tickets as $ticket) : ?>
					<div class="col-md-4">
						<div class="card border-primary mb-3">
						  <div class="card-header"  style="background-color: #343758; color: #fff;">
						  <?= $ticket->ticket_number ?> : <?= strtoupper($ticket->category) ?>
						  	</div>
						  <div class="card-body text-primary">
						    <h5 class="card-title"><?= $ticket->subject ?></h5>
						    <p class="card-text">
						    	<span class="fa fa-users"></span> Assigned to: <?= $ticket->a_lastname ?>, <?= $ticket->a_firstname?><br/>
						    	<span class="fa fa-flag"></span> Status: <?php if($ticket->ticket_status == 0){
						    		echo "IN PROCESS";
						    	} else { echo "CLOSED"; } ?> <br/>
						    	
						    </p>
						    <a class="btn btn-block btn-primary" href="<?=  base_url('outbound/ticket_details/'.$ticket->ticket_id); ?>"><span class="fa fa-eye"></span> View</a>
						  </div>
						</div>
					</div>
				<?php endforeach; ?>
