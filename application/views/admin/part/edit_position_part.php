	        ...
	        <div class="col-md-6 mb-3">
	         ...
	         <div class="col-md-6 mb-3">
	            ...
	            <select class="custom-select d-block w-100" id="status" name="status" required>
	              	<option value="0" <?php if($position_data->status == 0) { echo 'selected'; }  ?>>ACTIVATE</option>
	              	<option value="1" <?php if($position_data->status == 1) { echo 'selected'; }  ?>>DEACTIVATE</option>
	            </select>
	        </div>
		...
		<div class="modal fade" id="cancel_position_confirm" tabindex="-1" role="dialog" aria-hidden="true">
		  <div class="modal-dialog modal-dialog-centered" role="document">
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
		        Updating of position will not be saved, do you wish to continue?
		      </div>
		      <div class="modal-footer">
		        <button type="button" onClick="alert('Updating of position has been successfully cancelled.'); location.reload();" data-dismiss="modal" class="btn btn-primary">Yes</button>
		        <button type="button" class="btn btn-secondary" data-dismiss="modal">No</button>
		      </div>
		    </div>
		  </div>
